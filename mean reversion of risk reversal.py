# -*- coding: utf-8 -*-
"""
Created on Fri Feb 24 14:02:02 2023
...
def volsurface_builder():
    for y in ['1m','2m']:
        ...
        for x in np.arange(45,55):
            ...
            mkt_env = MarketEnvironment(t="23-02-2023", r=riskfree_rate, S_t= input_table['last'][0], sigma=vol_start) #random date( as long as difference in T-t is 1month or 2 months
           
            if y == '1m':
                time_expiry="23-03-2023"
            else:
                time_expiry="23-04-2023"    
            ...
            #put vol
            #delta_table_call.loc[y,x]= delta(r=riskfree_rate, S= input_table['last'][0],sigma=vol_start)
...
if period == 1:
    ...
    while int(tick1 < 280):
        ...
        for ax in axes.flat:
            ax.label_outer()
        plt.show()
        riskreversal_timeseries = []
        riskreversal_timeseries.append(float(vol_surface_put.loc['1m',left_wing]) - float(vol_surface_call.loc['1m',right_wing]))
        #trade left wing risk reversal
        if  float(vol_surface_put.loc['1m',left_wing]) - float(vol_surface_call.loc['1m',right_wing]) > float(np.mean(riskreversal_timeseries)): #+ float(2*np.std(riskreversal_timeseries)):
            #and float(option_data[option_data["ticker"] == str('RTM1P'+str(int(left_wing)))]["last"]) - float(option_data[option_data["ticker"] == str('RTM1C'+str(int(right_wing)))]["last"]) - 2*2 > 0 
            smile_1m_signal = 1
            exec1 = {'ticker': str('RTM1P'+str(int(left_wing))), 'type': 'MARKET', 'quantity': 25,'action': 'SELL'}
            exec2 = {'ticker': str('RTM1C'+str(int(right_wing))), 'type': 'MARKET', 'quantity': 25,'action': 'BUY'}
            execute(exec1)
            execute(exec2)
            mkt_env = MarketEnvironment(t="23-02-2023", r=riskfree_rate, S_t= input_table['last'][0], sigma=vol_start) #random date( as long as difference in T-t is 1month or 2 months
            Vanilla_Call = PlainVanillaOption(mkt_env,option_type='call', K= right_wing, T="23-03-2023")
            Vanilla_Put = PlainVanillaOption(mkt_env,option_type='put', K= left_wing, T="23-03-2023")
            delta_call = Vanilla_Call.delta()
            delta_put =  Vanilla_Put.delta()
            net_delta = float(delta_put) + float(delta_call)
            ...
            exec3 = {'ticker': 'RTM', 'type': 'MARKET', 'quantity': int(round(abs(net_delta)*100*25)),'action': y}
            execute(exec3)
            
                
             
        elif float(vol_surface_put.loc['1m',left_wing]) - float(vol_surface_call.loc['1m',right_wing]) < float(np.mean(riskreversal_timeseries)): #+ float(np.std(riskreversal_timeseries) * 2):
             #and float(option_data[option_data["ticker"] == str('RTM1P'+str(int(left_wing)))]["last"])*-1 + float(option_data[option_data["ticker"] == str('RTM1C'+str(int(right_wing)))]["last"]) - 2*2 > 0 :
            # exit positions
            smile_1m_signal = 1
            exec1 = {'ticker': str('RTM1P'+str(int(left_wing))), 'type': 'MARKET', 'quantity': 25,'action': 'BUY'}
            exec2 = {'ticker': str('RTM1C'+str(int(right_wing))), 'type': 'MARKET', 'quantity': 25,'action': 'SELL'}
            execute(exec1)
            execute(exec2)
            mkt_env = MarketEnvironment(t="23-02-2023", r=riskfree_rate, S_t= input_table['last'][0], sigma=vol_start) #random date( as long as difference in T-t is 1month or 2 months
            Vanilla_Call = PlainVanillaOption(mkt_env,option_type='call', K= right_wing, T="23-03-2023")
    ...
    while int(279 < tick1 < 300):
        # exit positions
        smile_1m_signal = 3
        positions = input_table[input_table.position!=0][['ticker','position']]
    ...
    while int(tick1 < 280):
        ...
        riskreversal_timeseries2 = []
        riskreversal_timeseries2.append(float(vol_surface_put.loc['2m',left_wing]) - float(vol_surface_call.loc['2m',right_wing]))
        
         #trade left wing risk reversal
        if  float(vol_surface_put.loc['2m',left_wing]) - float(vol_surface_call.loc['2m',right_wing]) > float(np.mean(riskreversal_timeseries2)) + float(2*np.std(riskreversal_timeseries2)):
            #and float(option_data[option_data["ticker"] == str('RTM1P'+str(int(left_wing)))]["last"]) - float(option_data[option_data["ticker"] == str('RTM1C'+str(int(right_wing)))]["last"]) - 2*2 > 0 :
            smile_2m_signal = 1
            exec1 = {'ticker': str('RTM2P'+str(int(left_wing))), 'type': 'MARKET', 'quantity': 25,'action': 'SELL'}
            exec2 = {'ticker': str('RTM2C'+str(int(right_wing))), 'type': 'MARKET', 'quantity': 25,'action': 'BUY'}
            execute(exec1)
            execute(exec2)
            mkt_env = MarketEnvironment(t="23-02-2023", r=riskfree_rate, S_t= input_table['last'][0], sigma=vol_start) #random date( as long as difference in T-t is 1month or 2 months
            Vanilla_Call = PlainVanillaOption(mkt_env,option_type='call', K= right_wing, T="23-04-2023")
            Vanilla_Put = PlainVanillaOption(mkt_env,option_type='put', K= left_wing, T="23-04-2023")
            delta_call = Vanilla_Call.delta()
            delta_put =  Vanilla_Put.delta()
            net_delta = float(delta_put) + float(delta_call)
        ...
        elif float(vol_surface_put.loc['2m',left_wing]) - float(vol_surface_call.loc['2m',right_wing]) < float(np.mean(riskreversal_timeseries2)) + float(2*np.std(riskreversal_timeseries2)):
             #and float(option_data[option_data["ticker"] == str('RTM2P'+str(int(left_wing)))]["last"])*-1 + float(option_data[option_data["ticker"] == str('RTM2C'+str(int(right_wing)))]["last"]) - 2*2 > 0 :
            # exit positions
             smile_2m_signal = 1
             exec1 = {'ticker': str('RTM2P'+str(int(left_wing))), 'type': 'MARKET', 'quantity': 25,'action': 'BUY'}
             exec2 = {'ticker': str('RTM2C'+str(int(right_wing))), 'type': 'MARKET', 'quantity': 25,'action': 'SELL'}
             execute(exec1)
             execute(exec2)
             mkt_env = MarketEnvironment(t="23-02-2023", r=riskfree_rate, S_t= input_table['last'][0], sigma=vol_start) #random date( as long as difference in T-t is 1month or 2 months
             Vanilla_Call = PlainVanillaOption(mkt_env,option_type='call', K= right_wing, T="23-04-2023")
